...
from experiments.models import SelfAttMLP

from utils.utils import check_makedir, model_summary, training_output
from utils.parsers import get_arg_array, MlpParser  # Parser TBD
...
# ########## ARGUMENT PARSER ########## #
args, args_dir_str = MlpParser().parsers()
DEVICE = torch.device("cuda:" + args.gpu_id)
DEPTH = int(args.depth)
WIDTH = get_arg_array(args.width)
...
MODEL_SAVE_PATH = os.path.join(OUTPUT_COLLECTION_PATH, "att_mlp")

network_args = {"device": DEVICE, "input_size": INPUT_SIZE, "depth": DEPTH,
                "width":WIDTH, "output_size": OUTPUT_SIZE, "seq_len": SEQ_LEN,
                "embed":False, "embed_size":4}
...
for plch in LAUNCH_POWER_RANGE:
    ...
    for fold in range(1, NUM_CROSS_VAL_FOLDS + 1):
        ...
        # ########## SET UP MODEL ########## #
        model = SelfAttMLP(**network_args).to(DEVICE)
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            trainer_args = {
                ...
                "attention": True,   #  Change to True for attention, False for no attention
            }
            ...
            val_args = {
                ...
                "loss_function": loss_function,
                "debug_mode": DEBUG_MODE,
                "attention": True,   #  Change to true afterwards
            }

            # loss, ber, q, _ = test_model(**val_args)
            loss, ber, q, att = test_model(**val_args)
            ...
            loss, ber, q, att = test_model(**test_args)
