"""Attention layer added to RNN layer to extract context features"""
# Internal
...
from experiments.models import SimpAttLayerRecurrent

from utils.utils import check_makedir, model_summary, training_output
from utils.parsers import get_arg_array, SimpAttLayerRnnParser
...
# ########## ARGUMENT PARSER ########## #
args, args_dir_str = SimpAttLayerRnnParser().parsers()
...
RTN_SEQ = bool(args.return_sequence == 'true')
...
MODEL_SAVE_PATH = os.path.join(OUTPUT_COLLECTION_PATH, "simp_att_layer")

network_args = {"device": DEVICE, "rnn_type": RNN, "n_hiddens": N_HIDDENS, "seq_len": SEQ_LEN,
                "bidirectional": BI, "unpack_bidir": UNPACK_BIDIR, "n_layers": N_LAYERS,
                "input_size": INPUT_SIZE, "output_size": OUTPUT_SIZE, "return_sequence": RTN_SEQ}
...
for plch in LAUNCH_POWER_RANGE:
    ...
    for fold in range(1, NUM_CROSS_VAL_FOLDS + 1):
        ...
        # ########## SET UP MODEL ########## #
        model = SimpAttLayerRecurrent(**network_args).to(DEVICE)
