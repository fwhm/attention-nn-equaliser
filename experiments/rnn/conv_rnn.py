"""trimming rnn outputs before flattening layer, only trim_tap [1, 2] will go through the flattening and dense layer"""
# Internal
...
from experiments.models import ConvRnn

from utils.utils import check_makedir, model_summary, training_output
from utils.parsers import get_arg_array, ConvRnnParser
...
# ########## ARGUMENT PARSER ########## #
args, args_dir_str = ConvRnnParser().parsers()
...
IN_TAP_IDs = get_arg_array(args.in_tap_ids)
OUT_TAP_IDs = get_arg_array(args.out_tap_ids)
...
MODEL_SAVE_PATH = os.path.join(OUTPUT_COLLECTION_PATH, "conv_rnn")

network_args = {"device": DEVICE, "rnn_type": RNN, "n_hiddens": N_HIDDENS,
                "seq_len": SEQ_LEN, "bidirectional": BI, "n_filters": N_FILTERS, "kernel_size": KERNEL_SIZE,
                "input_size": INPUT_SIZE, "output_size": OUTPUT_SIZE,
                "in_tap_ids": IN_TAP_IDs, "out_tap_ids": OUT_TAP_IDs}
...
for plch in LAUNCH_POWER_RANGE:
    ...
    for fold in range(1, NUM_CROSS_VAL_FOLDS + 1):
        ...
        # ########## SET UP MODEL ########## #
        model = ConvRnn(**network_args).to(DEVICE)
