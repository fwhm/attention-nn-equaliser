...
from experiments.models import TrimRnnOut

from utils.utils import check_makedir, model_summary, training_output
from utils.parsers import get_arg_array, TrimRnnOutParser
...
MODEL_SAVE_PATH = os.path.join(OUTPUT_COLLECTION_PATH, "trim_rnn")

network_args = {"device": DEVICE, "rnn_type": RNN, "n_hiddens": N_HIDDENS, "trim_tap": TRIM_TAP,
                "seq_len": SEQ_LEN, "bidirectional": BI, "n_layers": N_LAYERS,
...
for plch in LAUNCH_POWER_RANGE:
    ...
    for fold in range(1, NUM_CROSS_VAL_FOLDS + 1):
        ...
        # ########## SET UP MODEL ########## #
        model = TrimRnnOut(**network_args).to(DEVICE)
