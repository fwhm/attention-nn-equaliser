"""Trim input and output of RNN based on tap index. """
# Internal
...
from experiments.models import InOutTapRnn

from utils.utils import check_makedir, model_summary, training_output
from utils.parsers import get_arg_array, InOutTapRnnParser
...
# ########## ARGUMENT PARSER ########## #
args, args_dir_str = InOutTapRnnParser().parsers()
...
MODEL_SAVE_PATH = os.path.join(OUTPUT_COLLECTION_PATH, "rnn_in_out_taps")

network_args = {"device": DEVICE, "rnn_type": RNN, "n_hiddens": N_HIDDENS, "bidirectional": BI,
...
for plch in LAUNCH_POWER_RANGE:
    ...
    for fold in range(1, NUM_CROSS_VAL_FOLDS + 1):
        ...
        # ########## SET UP MODEL ########## #
        model = InOutTapRnn(**network_args).to(DEVICE)
