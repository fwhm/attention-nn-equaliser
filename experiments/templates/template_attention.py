...
from experiments.models import SimpAttentionRecurrent

from utils.utils import check_makedir, model_summary, training_output
from utils.parsers import get_plch_array, SimpAttRnnParser
...
# ########## ARGUMENT PARSER ########## #
args, arg_str_plch, args_dir_str = SimpAttRnnParser().parsers()
...
LAUNCH_POWER_RANGE = get_plch_array(arg_str_plch)
...
MODEL_SAVE_PATH = os.path.join(OUTPUT_COLLECTION_PATH, "template_attention")

network_args = {"device": DEVICE, "rnn": RNN, "n_hiddens": N_HIDDENS,
                "seq_len": SEQ_LEN, "bidirectional": BI, "n_layers": N_LAYERS,
                "input_size": INPUT_SIZE, "output_size": OUTPUT_SIZE,
                "d_model": N_HIDDENS, "n_heads": N_HEADS}
...
for plch in LAUNCH_POWER_RANGE:
    ...
    for fold in range(1, NUM_CROSS_VAL_FOLDS + 1):
        ...
        # ########## SET UP MODEL ########## #
        model = SimpAttentionRecurrent(**network_args).to(DEVICE)
